...
export function IntegrationsPage({ principal }: { principal: Principal }) {
  ...
  return <><PageTitle title="Integrations" description="Understand the capabilities and limits of each connected security tool."><Refresh loading={integrations.loading || aws.loading} onClick={() => { void integrations.refresh(); void aws.refresh(); void refreshProviders(); }} /></PageTitle><ErrorNotice message={integrations.error || aws.error || providerError || error} />
    ...
    <section className="panel stack"><div className="section-heading"><h2>AWS security</h2><Status value={!readAws ? 'Access restricted' : aws.loading ? 'Loading' : aws.error ? 'Unknown' : aws.data.configured !== true ? aws.data.status || 'Not configured' : aws.data.checking === true ? 'Checking' : report.status || aws.data.status || 'Not checked'} /></div>
      <p>Automatic host enrollment verifies identity at startup. Collection runs only when explicitly requested; opening or refreshing this page makes no AWS calls.</p>
      <p>Guardian does not sign you into AWS. Authenticate outside the app on the machine running Guardian, as the same operating-system user. For an SSO profile, run <code>aws sso login --profile YOUR_PROFILE</code> and complete the browser sign-in.</p>
      <details className="evidence"><summary>AWS login and profile setup</summary><div className="stack">
        <p>Replace <code>YOUR_PROFILE</code> with your AWS CLI profile name. Use the profile reported in this panel when available. For an existing SSO profile, run these commands in a terminal on the Guardian host:</p>
        <pre><code>{'aws sso login --profile YOUR_PROFILE\naws sts get-caller-identity --profile YOUR_PROFILE'}</code></pre>
        <p>Complete the browser sign-in, then check that the identity command reports the intended account and ARN. Environment credentials and workload roles use their own credential setup; they do not require SSO login.</p>
        <p>Select the profile in the same terminal that launches Guardian. PowerShell:</p>
        <pre><code>{'$env:AWS_PROFILE = "YOUR_PROFILE"'}</code></pre>
        <p>macOS or Linux:</p><pre><code>{'export AWS_PROFILE="YOUR_PROFILE"'}</code></pre>
        <p><code>GUARDIAN_AWS_PROFILE</code> overrides <code>AWS_PROFILE</code>. Without a named profile, Guardian uses the SDK default credential chain; it does not automatically select another profile just because you signed into it. Environment credentials can also take precedence over profile credentials, so verify the account shown here.</p>
        <p>Configure an inventory region in the selected profile or set <code>AWS_REGION</code>. The SSO login region is separate. Start Guardian with Node.js 24.14 or later, retaining your existing port and data directory.</p>
        <p>If startup enrollment failed, or you change the account, region or profile, restart Guardian after setup. A restart ends sessions and clears memory-only AI provider keys. If an enrolled account's SSO session expires, renew that same account's login and retry <strong>Check AWS</strong>. Refresh only reads local status; it does not sign in or enroll another account.</p>
        <p>A verified identity does not guarantee access to every service. GuardDuty <code>NotEnabled</code> or Security Hub <code>InvalidAccessException</code> can indicate service setup or permission gaps. Review the reported coverage and account permissions; Guardian does not enable these services for you.</p>
      </div></details>
      {readAws ? <><Facts value={{ credentialMode: aws.data.mode === 'host_cli' ? 'Host AWS credentials' : aws.data.mode === 'pinned' ? 'Pinned enrollment' : undefined, account: aws.data.accountId, region: aws.data.region, profile: aws.data.profile, identity: aws.data.identityOk === true ? 'Verified' : 'Not currently verified', lastVerified: aws.data.lastIdentityAt ? dateLabel(aws.data.lastIdentityAt) : undefined, collected: report.collectedAt ? dateLabel(report.collectedAt) : 'Not recorded' }} /><button disabled={busy || aws.loading || !collectAws || aws.data.configured !== true || aws.data.checking === true} onClick={() => void checkAws()}>{busy || aws.data.checking === true ? 'Checking AWS...' : 'Check AWS'}</button>{!aws.loading && !aws.error && <p className="muted">{label(aws.data.message, 'AWS status has not been reported.')}</p>}{asList(report.coverage).map(item => <div key={label(item.id)}><div className="section-heading"><strong>{label(item.name || item.id)}</strong><Status value={item.status} /></div><p>{label(item.description)}</p></div>)}{Array.isArray(report.errors) && report.errors.length > 0 && <div className="notice warning"><ul>{report.errors.map((item, index) => <li key={index}>{label(item)}</li>)}</ul></div>}{Object.keys(report).length > 0 && <Evidence value={report} title="AWS collection evidence and coverage" />}</> : <p className="muted">AWS evidence requires installation scope and cloud:read. Collection additionally requires cloud:collect and an operator or administrator session.</p>}
    </section>
    <section className="panel stack"><div className="section-heading"><h2>Microsoft Entra ID</h2><Status value={entra === null ? 'Unknown' : entra ? 'Configured' : 'Not configured'} /></div><p>{entra === true ? 'Enterprise sign-in is enabled by this Guardian service. Account access remains subject to its tenant and role configuration.' : entra === false ? 'Enterprise sign-in is not enabled on this Guardian service.' : 'Identity provider availability has not been confirmed.'}</p><p>Entra signs users into Guardian through your organization's identity provider. It does not sign you into AWS or grant Azure or Microsoft 365 inventory access.</p></section>
