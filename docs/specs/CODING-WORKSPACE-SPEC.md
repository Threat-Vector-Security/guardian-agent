...
- `CodeSessionWorkState`
  ...
  - `compactedSummary`
  - `workflow`
    - includes the current recipe/stage plus any configured isolation recommendation for bounded higher-risk jobs
  - `workspaceProfile`
  - `workspaceTrust`
...
- the `Activity` panel also surfaces the active workflow recipe, current stage, verification posture, and any configured remote-isolation recommendation for bounded jobs so the operator can see whether the run is still inspecting, implementing, verifying, or ready to summarize
