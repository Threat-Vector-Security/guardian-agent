# Specification: Dify-Inspired Improvements
...
- Centralized `isPrivateAddress()` function replacing two duplicate `isPrivateHost` functions (from `src/tools/executor.ts` and `src/tools/browser-session.ts`)
