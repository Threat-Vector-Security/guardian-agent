...
Guardian uses the host's AWS SDK credential chain by default: environment credentials, shared AWS configuration/credentials, cached SSO sessions, or instance/workload roles. It uses the same local profiles and sessions you configure for the AWS CLI; the CLI executable is not required by Guardian. Start Guardian as the operating-system user whose AWS configuration and session you intend to use.

**Integrations → AWS login and profile setup** explains authentication even when AWS is healthy. Guardian does not sign you into AWS. For an existing SSO profile, run `aws sso login --profile YOUR_PROFILE`, complete the browser sign-in, then run `aws sts get-caller-identity --profile YOUR_PROFILE` to confirm the account and ARN. In the terminal used to start Guardian, select the profile with `$env:AWS_PROFILE = "YOUR_PROFILE"` (PowerShell) or `export AWS_PROFILE="YOUR_PROFILE"` (macOS/Linux). Guardian does not automatically select another profile because it has an active SSO session. Environment credentials can take precedence over profiles; verify the account reported in Integrations. Use Node.js 24.14 or later. Workload roles and environment credentials do not require SSO login.

At startup, Guardian resolves the region from `AWS_REGION`, then `AWS_DEFAULT_REGION`, then the selected profile's region. Profile selection is `GUARDIAN_AWS_PROFILE`, then `AWS_PROFILE`, then the SDK default. Setting only `GUARDIAN_AWS_PROFILE` is supported. The SSO login region is not an inventory region; Guardian does not guess one when none is configured.

Without explicit account/region pins, a bounded startup STS identity request discovers the account. **Integrations** then shows **Host AWS credentials**, the account/region and last successful identity verification. The discovered account and region are pinned in memory for this process lifetime, and every collection verifies STS again. Opening or refreshing Integrations reads local status; only **Check AWS**, **Environments → Collect now**, or `aws.check.start` requests inventory/finding collection. Startup discovery does not enable periodic AWS collection.

For explicit enrollment, set both `GUARDIAN_AWS_ACCOUNT_ID` (twelve digits) and `GUARDIAN_AWS_REGION`. **Integrations** shows **Pinned enrollment**. These pins override region discovery and require an exact STS account match before inventory calls. Setting only one pin is a startup error. `GUARDIAN_AWS_PROFILE` remains an optional profile override. Existing pinned setups continue to work.

If the region is missing, Integrations reports **Needs region**. If credentials are absent or the SSO session has expired at startup, Guardian still starts and AWS reports **Unavailable** with recovery instructions. For SSO, run `aws sso login --profile YOUR_PROFILE` on the host; set the profile's region or `AWS_REGION`, then restart Guardian if startup discovery failed. After a successful enrollment, renewing the same account's session and selecting **Check AWS** retries identity verification without rebinding the account. A different account is rejected; changes to account/region/profile selection require a restart.

Guardian does not accept AWS access keys or arbitrary endpoints through its UI or configuration objects, and does not persist AWS credentials or a discovered enrollment. Normal collected account/region evidence still persists with its observations. Restart with `node dist/security-main.js serve` and your existing `--port` and `--data-dir` options. A restart ends sessions and clears memory-only AI provider keys.

The read-only policy is `policies/aws-security-readonly.json`. Collection verifies STS identity against the explicitly pinned or startup-discovered account before requesting EC2 instances/security groups, Security Hub findings and GuardDuty findings. Service endpoints stay pinned, and account/region ownership filters exclude foreign inventory or aggregated findings. Disabled services, missing permissions, timeouts and bounded/truncated results remain visible as incomplete coverage. A verified identity does not guarantee service access: GuardDuty `NotEnabled` or Security Hub `InvalidAccessException` can indicate service setup or permission gaps. It does not enable services, change firewall rules, quarantine resources or remediate findings. One account and region are enrolled per local instance. Use `cloud:read` and `cloud:collect` scopes for assistants; project-restricted credentials cannot inspect account-wide cloud state. Verify IAM and collection in the intended customer account before operational acceptance.
