...
- Brokered delegation now writes `delegated_worker_started`, `delegated_worker_running`, `delegated_worker_completed`, `delegated_worker_failed`, and `delegated_worker_followup_action` rows into the routing trace, so use that file first when you need to confirm whether Guardian handed work to another agent, whether the worker ever started, whether it blocked on approval or failed outright, and whether a held result was replayed, kept held, or dismissed. Delegated run-class metadata is policy-owned: explicit/background delegated runs may be held for operator review, while ordinary code-session answers remain inline. For follow-up rows, web and CLI actions include the acting user/principal/surface fields when the channel supplied them.
