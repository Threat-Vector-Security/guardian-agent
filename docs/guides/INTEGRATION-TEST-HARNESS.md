...
- Brokered delegation now writes `delegated_worker_started`, `delegated_worker_running`, `delegated_worker_completed`, `delegated_worker_failed`, and `delegated_worker_followup_action` rows into the routing trace, so use that file first when you need to confirm whether Guardian handed work to another agent, whether the worker ever started, whether it blocked on approval or failed outright, and whether a held result was replayed, deferred, or dismissed. Delegated run-class metadata is policy-owned: explicit/background delegated runs may be held for operator review, while ordinary code-session answers remain inline. For follow-up rows, web and CLI actions include the acting user/principal/surface fields when the channel supplied them; deferred follow-up rows also include `deferredUntil` when the operator supplied a review time.
- The web `System > Runtime Execution` view should also show matching live handoff entries such as `Delegated to …`, `… is working`, and the final blocked/completed status for the same run. When isolating delegated work, filter `/api/assistant/runs` or the Runtime Execution controls by `runClass`, `reportingMode`, and `unresolvedBlockerKind` alongside execution lineage filters.
...
| **`scripts/test-policy-update-visibility.mjs`** | Policy update visibility harness: strict degraded sandbox catalog exposure plus domain/path allowlist approval flows in in-process and brokered modes (Node.js) | focused policy visibility assertions |
...
| **`scripts/test-memory-surface.mjs`** | Memory surface harness: `memory_save`, memory wiki CRUD, audit/lint exposure, code-session listing, and maintenance endpoint behavior (Node.js) | focused memory surface assertions |
...
| **`scripts/test-brokered-worker-smoke.mjs`** | Brokered worker entry smoke harness: direct worker JSON-RPC startup, supervisor LLM callback handling, and non-stub response validation (Node.js) | focused worker-entry assertions |
...
| **`scripts/test-remediation-stress.mjs`** | Remediation stress harness: critical metadata deletion denial, recursive delete approval gating, and expanded search-result formatting (Node.js with `tsx`) | focused remediation assertions |
