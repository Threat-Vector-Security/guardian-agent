...
For tier-routed chat, the built-in `local` and `external` agents share one logical memory/session identity. Switching between `auto`, `local-only`, and `external-only` changes the execution backend, not the assistant's conversation history or knowledge base.
...
Layer 1: Knowledge Base (AgentMemoryStore)
  Per-agent markdown files plus sidecar metadata (~/.guardianagent/memory/{agentId}.md + .index.json)
  Only active/reviewed content is loaded into normal LLM context
  Written via memory_save tool or automatic memory flush with trust/provenance metadata
...
- the built-in dual-agent `local` / `external` assistant shares one conversation/session state key and one knowledge base
- explicitly distinct user-defined agents still keep separate memory unless you intentionally design them otherwise
...
- quarantined entries are persisted in sidecar metadata but excluded from normal planner context
- verification distinguishes active writes from quarantined/unreviewed writes
- `memory_search` and `memory_recall` only expose quarantined material when explicitly requested through operator-visible inactive search paths

## Knowledge Base File Format

Each agent's active knowledge base is a markdown view organized by category:
...
Structured trust metadata is stored alongside the markdown in a sidecar index. Quarantined, expired, or rejected entries stay out of the markdown view so poisoned remote content cannot silently re-enter the planner as durable context.

For the built-in tier-routed assistant, the `local` and `external` execution agents write into the same logical knowledge base so mode switches do not fork remembered context.
...
1. ~~**Vector embeddings**~~: **Done** — native search pipeline provides BM25 + vector similarity via `doc_search` tool (`assistant.tools.search`)
...
| `src/tools/executor.ts` | memory_search, memory_recall, memory_save + doc_search, doc_search_status, doc_search_reindex tool registration |
| `src/tools/types.ts` | 'memory' and 'search' tool category definitions |
| `src/config/types.ts` | AssistantKnowledgeBaseConfig, SearchConfig types |
| `src/search/` | Native search pipeline (SearchService, DocumentStore, FTSStore, VectorStore, HybridSearch) |
| `src/index.ts` | Service wiring, memory flush callback, ChatAgent KB injection, search bootstrap |
