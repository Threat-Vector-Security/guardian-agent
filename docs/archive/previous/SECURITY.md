...
The default 2.0 alpha product follows [the security conversion architecture](docs/architecture/SECURITY-CONVERSION.md) and [operator guide](../../guides/SECURITY-WORKSPACE.md). It uses a loopback security service, scoped assistant credentials, separate administrative sessions, durable jobs and ContextCypher workspace revisions. It does not start the legacy assistant runtime described below. The previous architecture is retained for shared-component regression coverage and explicit legacy migration use.
