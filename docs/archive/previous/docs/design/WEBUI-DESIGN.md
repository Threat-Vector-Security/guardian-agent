...
Configuration opens as a modal window in focused mode so setup stays available without displacing the main chat surface. Other navigation entries may use the same modal window pattern when selected from the rail. Focused page windows should be movable, resizable, maximizable, and locally restore their last size, position, and stacking state so large tools such as Code can keep using the full available workspace. Double-clicking the focused page-window titlebar resets its saved size and position. Maximized focused windows must stay inside the usable route-window area so their titlebar, maximize control, and close control remain reachable. Closing the modal returns the operator to the chat-first surface. Classic and workstation shells remain selectable from Configuration > Appearance and must be restorable without reload.
...
The active shell layout should also be switchable from a compact top-right selector so operators can move between Default Chat, Classic, and Workstation layouts without opening Configuration.
...
- guided setup-card visibility/state for the Second Brain home surface
...
- placeholder or duplicate preferences that do not affect shipped Second Brain behavior
