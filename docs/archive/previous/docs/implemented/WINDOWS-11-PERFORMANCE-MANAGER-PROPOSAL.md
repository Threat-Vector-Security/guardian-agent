...
- optional extension of [native/windows-helper/src/main.rs](https://github.com/Threat-Vector-Security/guardian-agent/blob/v1.0.0/native/windows-helper/src/main.rs) for Win32-only operations that are awkward from Node
...
`WEBUI-DESIGN.md` is the current source of truth. This proposal should not be implemented as a top-level page unless that spec is updated in the same change to formally introduce `Performance` into the canonical navigation and ownership model.
