...
| SSRF protection blocks localhost/private and obfuscated forms | `src/guardian/ssrf-protection.ts`, `src/tools/executor.ts`, `src/tools/builtin/web-tools.ts` | `src/guardian/ssrf-protection.test.ts`, `src/tools/executor.test.ts` | `scripts/test-security-verification.mjs` exercises `web_fetch` against decimal, hex, octal, loopback, and metadata endpoint URLs through `POST /api/tools/run` |
