...
| Generic non-brokered multi-step web continuation | **Real bug, fixed** | The generic web approval harness used to fall back into "You already have blocked work waiting for input or approval" after approval continuation. This was due to `continueDirectRouteAfterApproval` failing to clear the pending action (passing `pendingActionAlreadyCleared: true` by mistake), causing shared continuation-state drift. This has now been corrected. |
