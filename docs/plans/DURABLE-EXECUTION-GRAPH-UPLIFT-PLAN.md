...
- Remaining risks after this checkpoint: the suspended tool-loop artifact is graph-backed and not exposed through pending-action metadata, but it still snapshots model messages internally. Replacing those snapshots with explicit tool-observation/checkpoint artifacts remains future graph-controller work. Exact-file synthesis quality, richer graph/operator observability, delegated budget-persistence ownership, and broader connector mutation-plus-approval sweeps remain open cleanup areas.
