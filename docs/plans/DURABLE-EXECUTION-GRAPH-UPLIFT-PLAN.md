...
6. Continue delegated graph cleanup by moving any remaining delegated retry/recovery ownership into graph node runners/controllers and deleting overlapping WorkerManager side channels as each path is proven. The pure retry, handoff, verification, recovery-advisor schema, and recovery-advisor invocation helpers have already moved; do not keep extracting helper fragments unless the slice deletes a real overlapping owner or clarifies a graph boundary.
