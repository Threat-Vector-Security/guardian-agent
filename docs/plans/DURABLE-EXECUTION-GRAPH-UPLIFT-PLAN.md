...
6. Continue delegated graph cleanup by moving delegated retry/recovery invocation ownership into graph node runners/controllers and deleting overlapping WorkerManager side channels as each path is proven. The pure retry, handoff, verification, and recovery-advisor schema helpers have already moved; do not keep extracting helper fragments unless the slice deletes a real overlapping owner or clarifies a graph boundary.
