...
**Status:** Architecture refinement and verification phase. Phases 1-4 are implemented for the read-only graph/artifact lane and the first graph-controlled search/write slice. Phase 5+ approval/continuation and delegated graph cleanup are partially implemented. The latest app-facing slices proved the current routing, continuity, approval, security, OpenRouter/Ollama Cloud managed-cloud, browser tool-loop, web approval, skills-routing, and Code UI smoke paths against the actual app and browser/API surfaces. The current risk is no longer missing primitives; it is deleting the remaining overlapping owners as graph-owned replacements land and continuing delegated graph cleanup without weakening brokered-worker isolation.
**Date:** 2026-04-27
