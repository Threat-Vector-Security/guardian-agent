...
- Delegated graph ownership is narrower after the latest cleanup, and retry policy has moved into the execution-graph layer. Delegated worker dispatch, retry invocation/budget persistence, task-contract verification orchestration, and recovery-advisor invocation are still coordinated from `WorkerManager`. Continue moving those decisions into graph node/controller boundaries before deleting old side channels.
...
- Post-cleanup delegated live smoke is still an open risk, not a pass: request id `durable-redaction-live-smoke-42801` asked for automation count plus repo symbol search, exceeded the 240s client timeout, and the trace showed the delegated worker still `running` without tool evidence before the app was restarted to clear the stalled smoke run. This matches the remaining delegated live-loop/retry overlap rather than the trace-redaction or multi-approval fixes.
- Remaining risks after this checkpoint: the uplift is still not 100% complete. The routing trace redaction boundary is now covered, but run-timeline/detail artifact presentation still needs a separate audit before exposing richer connector payload drill-downs; delegated retry/recovery still has overlap between graph node runners and legacy worker-manager/controller paths; and broad live connector mutation sweeps with multiple independent approvals still need to be run after the delegated live-loop stall is addressed.
...
Context: Phases 1-4 are implemented. Phase 5+ is partially implemented. The latest architecture-refinement and app-facing slices scoped stale continuity/code-session state out of fresh surfaces, split automation authoring/control planned-step ownership, fixed non-stream `/api/message` request metadata, surface-scoped normal chat history, blocked policy expansion to sensitive Guardian config paths, rejected stale model-supplied `resolvedContent` for ordinary turns, normalized simple direct-assistant workload recovery back to direct answers, accepted compact dated OpenRouter/OpenAI snapshot model ids as aliases in delegated verification, and stopped low-confidence `unknown` Intent Gateway decisions from bypassing model-requested tool loops through the no-tool direct shortcut. This latest no-tool gate fix is route/confidence-state driven, not keyword/regex routing. Focused orchestration tests, npm run check, npm test, npm run build, Code UI smoke, web approval smoke, and the managed-cloud app API exact-answer sweep passed before this handoff. OpenRouter is still configured but may be blocked by account credits; use Ollama Cloud for managed-cloud live sweeps unless OpenRouter credits are restored. The remaining work is deletion of overlapping approval/delegated owners as graph-owned replacements land.
