...
- Phase 5 graph interrupts are implemented for the first approval/clarification slices and for brokered delegated-worker approval suspension/resume. A live OpenRouter API sweep proved pending-action creation, approval API decision, tool execution, and final continuation for a harmless policy-gated `fs_write`; several legacy producers have already been deleted, but live chat tool-loop resume plus the remaining delegated retry dispatch/budget-persistence ownership and basic broker polling still need graph-native replacement before their old owners can be removed.
...
- Current architecture cleanup still needed: make the remaining approval/resume payload executors and suspended tool-loop replay graph-native before deleting the replay owners; move delegated worker dispatch and retry invocation/budget persistence behind graph node/controller boundaries; continue shrinking `ChatAgent` direct-route/coding/Second Brain callback ownership; consolidate provider fallback/profile retry ownership in execution profile/runtime orchestration; finish System tab graph-timeline observability over graph/runtime events rather than parallel progress feeds.
...
- Delegated graph ownership is narrower after the latest cleanup: retry policy, retry-attempt planning, retry profile-selection policy, terminal handoff, result verification, verification-cycle orchestration, and extended evidence-drain policy have moved into the execution-graph layer. Delegated worker dispatch, retry invocation/budget persistence, and ToolExecutor job polling are still coordinated from `WorkerManager` because they require supervisor/runtime authority. Continue moving only graph-owned decisions into graph node/controller boundaries before deleting old side channels.
...
3. Continue delegated graph cleanup by moving retry invocation/budget persistence behind graph node/controller boundaries, deleting overlapping `WorkerManager` side channels as each path is proven.
...
6. Continue delegated graph cleanup by moving any remaining delegated retry/recovery ownership into graph node runners/controllers and deleting overlapping WorkerManager side channels as each path is proven. The pure retry, handoff, verification, recovery-advisor schema, recovery-advisor invocation, answer-only grounded synthesis retry invocation, extended evidence-drain orchestration, delegated retry profile-selection, terminal delegated verification finalization, and delegated verification-cycle helpers have already moved; do not keep extracting helper fragments unless the slice deletes a real overlapping owner or clarifies a graph boundary.
