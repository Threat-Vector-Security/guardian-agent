...
- Current architecture cleanup still needed: make the remaining approval/resume payload executors and suspended tool-loop replay graph-native before deleting the replay owners; move delegated worker dispatch, retry invocation/budget persistence, and task-contract verification orchestration behind graph node/controller boundaries; continue shrinking `ChatAgent` direct-route/coding/Second Brain callback ownership; consolidate provider fallback/profile retry ownership in execution profile/runtime orchestration; finish System tab graph-timeline observability over graph/runtime events rather than parallel progress feeds.
...
- Delegated graph ownership is narrower after the latest cleanup: retry policy, retry-attempt planning, retry profile-selection policy, terminal handoff, result verification, and extended evidence-drain policy have moved into the execution-graph layer. Delegated worker dispatch, retry invocation/budget persistence, and ToolExecutor job polling are still coordinated from `WorkerManager` because they require supervisor/runtime authority. Continue moving only graph-owned decisions into graph node/controller boundaries before deleting old side channels.
...
3. Continue delegated graph cleanup by moving retry invocation/budget persistence and task-contract verification orchestration behind graph node/controller boundaries, deleting overlapping `WorkerManager` side channels as each path is proven.
