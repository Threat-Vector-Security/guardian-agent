...
**Status:** Durable execution graph uplift implementation is complete for the scoped architecture plan. Phases 1-4 are implemented for the read-only graph/artifact lane and the first graph-controlled search/write slice. Phase 5+ approval/continuation and delegated graph cleanup are implemented for the proven core paths and app-proven across routing, continuity, approval, security, Ollama Cloud managed-cloud routing, browser/tool-loop reads, second-brain approvals, memory save/search, automation pagination, web approval, skills-routing, Code UI smoke paths, and a real-app multi-domain connector sweep across Vercel, WHM, Gmail, Microsoft 365, automations, and repo search. The cleanup moved delegated recovery-advisor invocation, answer-only grounded synthesis retry invocation, extended evidence-drain orchestration, delegated retry profile-selection policy, terminal delegated verification finalization, delegated verification-cycle orchestration, request-scoped delegated job snapshot/drain polling, delegated retry invocation sequencing, delegated automation-authoring chat-continuation resume terminalization, graph mutation approval-resume reconstruction, worker-suspension approval-resume reconstruction, the graph-controlled read/synthesize/mutate/verify runner, worker-suspension continuation approval pending-action projection, and delegated-worker approval pending-action projection into shared graph-owned modules while keeping broker dispatch, ToolExecutor execution, pending-action lifecycle, and ToolExecutor `listJobs` access in WorkerManager behind broker-safe callbacks. No remaining ownership cleanup is currently justified inside this plan without widening scope; synthesis/search quality pressure should continue as a separate refinement plan.
**Date:** 2026-04-29
...
- Current architecture cleanup still needed: make the remaining approval/resume payload executors and suspended tool-loop replay graph-native before deleting the replay owners; move delegated worker dispatch/budget persistence behind graph node/controller boundaries; continue shrinking `ChatAgent` direct-route/coding/Second Brain callback ownership; consolidate provider fallback/profile retry ownership in execution profile/runtime orchestration; finish System tab graph-timeline observability over graph/runtime events rather than parallel progress feeds.
...
- `src/runtime/chat-agent/chat-continuation-graph.ts` now owns failed chat-continuation graph terminalization through `failChatContinuationGraphResume`, and `startChatContinuationGraphApprovalResume` can still preserve caller-specific denial copy while emitting the shared graph-failed event and metadata shape.
- `WorkerManager` delegated automation-authoring approval resume now uses the shared chat-continuation graph start/complete/fail helpers instead of hand-emitting `interruption_resolved`, `graph_completed`, and `graph_failed` events. WorkerManager still owns supervisor/runtime authority for worker dispatch and automation authoring execution; it no longer owns this graph lifecycle projection.
...
- Natural-language repo search now has a deterministic content fallback in `fs_search` for zero-literal-match content searches, plus direct-reasoning required-search enforcement before synthesis. The live API replay for "where execution graph mutation approval resume events are emitted" now returns the `emitMutationResumeGraphEvent` definition in `src/runtime/execution-graph/mutation-node.ts` and a production call site in `src/supervisor/worker-manager.ts` instead of a broad raw search dump or a generic graph-event helper.
...
Checkpoint after the delegated evidence-drain ownership cleanup:

- `src/runtime/execution-graph/delegated-worker-verification.ts` now owns `runDelegatedEvidenceDrainExtension`: budget-exhaustion skip, extended-drain eligibility, 60s evidence-drain deadline, wait-expired trace intent, post-drain verification, and retryable insufficiency reconstruction.
- `WorkerManager` still owns broker polling through `ToolExecutor.listJobs`, but now supplies it as a broker-safe callback. It no longer has a private extended evidence-drain helper or inline extended-drain verification logic.
- Brokered-worker isolation remains intact: the graph verifier receives only metadata, task contract, verification state, job snapshots, and a polling callback, not direct `Runtime`, provider, channel, or filesystem authority.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/delegated-worker-verification.test.ts src/supervisor/worker-manager.test.ts` reported 54 passing tests.
- Full local gates passed after the extraction: `npm run check`, `npm run build`, and full `npm test` (314 files, 3403 tests).

Checkpoint after the delegated retry profile-selection cleanup:

- `src/runtime/execution-graph/delegated-worker-retry.ts` now owns `selectDelegatedRetryExecutionProfile`: managed-cloud sibling selection for missing tool evidence, escalated delegated profile fallback, and current-profile fallback when config is unavailable.
- `WorkerManager` supplies only the broker-safe inputs for the policy decision: config snapshot, orchestration descriptor, gateway decision, current selected profile, and retry insufficiency. It no longer carries a private delegated retry profile selector or imports execution-profile selection helpers for this path.
- Brokered-worker isolation remains intact: profile-selection policy receives config/metadata only, not direct `Runtime`, `ToolExecutor`, provider, channel, or filesystem authority.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/delegated-worker-retry.test.ts src/supervisor/worker-manager.test.ts` reported 58 passing tests.
- Full local gates passed after the cleanup: `npm run check`, `npm run build`, and full `npm test` (314 files, 3404 tests).

Checkpoint after the terminal delegated verification finalization cleanup:

- `src/runtime/execution-graph/delegated-worker-verification.ts` now owns `finalizeDelegatedWorkerVerification`: terminal verification attachment, old verification-event replacement, immutable verification payload copy, and task-contract reconciliation trace detail.
- `WorkerManager` now asks the graph verification module for the finalized envelope and trace detail, then only publishes the trace and persists completion metadata. It no longer has a private terminal verification attachment helper.
- This is a narrow ownership deletion, not a routing or retry behavior change. Brokered-worker isolation remains intact because the finalizer receives only the task contract, verified envelope/decision, and timestamp.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/delegated-worker-verification.test.ts src/supervisor/worker-manager.test.ts` reported 55 passing tests.
- Full local gates passed after the cleanup: `npm run check`, `npm run build`, and full `npm test` (314 files, 3405 tests).

Checkpoint after the delegated verification-cycle orchestration cleanup:

- `src/runtime/execution-graph/delegated-worker-verification.ts` now owns `runDelegatedWorkerVerificationCycle`: worker-result verification, task-contract adoption, retryable insufficiency construction, and optional extended evidence-drain retry through a broker-safe polling callback.
- `WorkerManager` still performs the basic delegated job polling through `ToolExecutor`, dispatches the initial and retry worker turns, and records audit/progress, but no longer duplicates the verify/adopt/extended-drain cycle for first and retry attempts.
- Brokered-worker isolation remains intact: the cycle helper receives metadata, task contract, job snapshots, execution profile, and callbacks only, not direct `Runtime`, provider, channel, or filesystem authority.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/delegated-worker-verification.test.ts src/supervisor/worker-manager.test.ts` reported 56 passing tests.
- Full local gates passed after the cleanup: `npm run check`, `npm run build`, and full `npm test` (314 files, 3406 tests).

Checkpoint after the delegated job-drain ownership cleanup:

- `src/runtime/execution-graph/delegated-worker-verification.ts` now owns request-scoped delegated job snapshot listing and basic drain polling through `listDelegatedRequestJobSnapshots` and `awaitDelegatedRequestJobDrain`.
- `WorkerManager` no longer has private delegated job snapshot or drain helpers. It supplies only a broker-safe `listJobs` callback, preserves the prior no-snapshot behavior when a test broker does not expose `listJobs`, and continues to own worker dispatch, audit, trace, and progress publication.
- This deletes the overlap between basic delegated job polling and extended evidence-drain orchestration without granting the brokered worker direct `Runtime`, `ToolExecutor`, provider, channel, or filesystem authority.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/delegated-worker-verification.test.ts src/supervisor/worker-manager.test.ts` reported 57 passing tests.
- Full local gates passed after the cleanup: `npm run check`, `npm run build`, and full `npm test` (314 files, 3407 tests).
- Rebuilt-app API spot check passed on the managed-cloud path: `GET /api/status`, exact marker request `codex-live-drain-exact-42804`, mixed web+repo delegated request `codex-live-drain-mixed-42804` with graph status `completed` and verifier decision `satisfied`, and raw credential disclosure refusal `codex-live-drain-security-42804`.

Checkpoint after the delegated retry invocation cleanup:

- `src/runtime/execution-graph/delegated-worker-retry-invocation.ts` now owns `runDelegatedWorkerRetryInvocation`: retry-profile choice, retry-attempt plan construction, retry request construction callback ordering, retry dispatch callback invocation, post-retry job drain, drain-expired event intent, and retry verification callback invocation.
- `WorkerManager` supplies only broker-safe callbacks for worker dispatch, delegated job drain, retry verification, trace/progress publication, and audit recording. It no longer inlines the retry profile/plan/dispatch/drain/verify sequence, and it still does not grant brokered workers direct `Runtime`, `ToolExecutor`, provider, channel, or filesystem authority.
- This is an ownership cleanup, not a behavior broadening: simple/direct routes, Intent Gateway classification, provider orchestration, and delegated-worker isolation are unchanged.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/delegated-worker-retry-invocation.test.ts src/runtime/execution-graph/delegated-worker-retry.test.ts src/runtime/execution-graph/delegated-worker-verification.test.ts src/supervisor/worker-manager.test.ts` reported 70 passing tests.
- Full local gates passed after the cleanup: `npm run check`, `npm run build`, and full `npm test` (315 files, 3409 tests).
...
Checkpoint after the mutation approval-result normalization cleanup:

- `src/runtime/execution-graph/mutation-node.ts` now owns `buildApprovedMutationToolResult`, the normalization that turns approved `ToolExecutor` decisions into the mutation resume receipt input.
- `WorkerManager` still owns approval decision handling, pending-action completion, graph event persistence, and the actual supervisor `ToolExecutor` callback. It no longer carries a local mutation approval-result normalizer.
- This is a mutation-resume ownership cleanup only. It does not change approval policy, pending-action resolution, filesystem authority, tool execution, routing, provider/profile selection, or brokered-worker isolation.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/mutation-node.test.ts src/supervisor/worker-manager.test.ts` reported 56 passing tests.
- Local gates passed after the cleanup: `npm run check` and `npm run build`.
- Cross-domain regression coverage passed after the cleanup: `node scripts/test-cross-domain-orchestration-stress.mjs`.
...
Checkpoint after the graph approval-resume reconstruction cleanup:

- `src/runtime/execution-graph/mutation-node.ts` now owns reconstruction of mutation approval-resume state from pending-action metadata, graph snapshots, and stored `WriteSpec` artifacts through `reconstructGraphMutationResume`.
- `src/runtime/execution-graph/worker-suspension-resume.ts` now owns reconstruction of delegated worker-suspension resume state from pending-action metadata, graph snapshots, and `WorkerSuspension` artifacts through `reconstructWorkerSuspensionGraphResume`.
- `WorkerManager` now supplies only the graph store and approval decision context before dispatching the reconstructed graph-owned resume context into supervisor-owned tool execution or brokered worker continuation. It no longer reconstructs graph suspension state inline.
- This is an approval/resume ownership cleanup only. It does not change Intent Gateway routing, provider/profile selection, approval policy, filesystem authority, pending-action lifecycle ownership, or brokered-worker isolation.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/worker-suspension-resume.test.ts src/runtime/execution-graph/mutation-node.test.ts src/supervisor/worker-manager.test.ts` reported 62 passing tests.
- Approval and continuity regression coverage passed: `npx vitest run src/runtime/continuity-threads.test.ts src/runtime/incoming-dispatch.test.ts src/runtime/direct-reasoning-mode.test.ts src/runtime/execution-graph/worker-suspension-resume.test.ts src/runtime/execution-graph/mutation-node.test.ts src/supervisor/worker-manager.test.ts` reported 121 passing tests.
- Local gates passed after the cleanup: `npm run check` and `npm run build`.
- Cross-domain and web approval harnesses passed after the cleanup: `node scripts/test-cross-domain-orchestration-stress.mjs` and `node scripts/test-web-approvals.mjs`.

Checkpoint after the graph-controlled runner ownership cleanup:

- `src/runtime/execution-graph/graph-controller.ts` now owns the graph-controlled read/synthesize/mutate/verify runner through `runGraphControlledExecution`: read-only gateway projection, graph shell creation, read-node dispatch, evidence artifact ingestion, write-spec synthesis, mutation execution, verification, approval interruption, and terminal response metadata.
- `WorkerManager` now supplies only supervisor-owned callbacks for worker acquisition, worker dispatch, code-session registry prompt context, `ToolExecutor` execution, pending-action storage, graph store, run timeline, runtime config, and the clock. It no longer inlines the graph-controlled execution sequence.
- This is an ownership cleanup only. It does not change Intent Gateway classification, planned-step routing rules, provider/profile selection, approval policy, filesystem authority, pending-action lifecycle ownership, or brokered-worker isolation.
- Focused graph-runner coverage passed: `npx vitest run src/runtime/execution-graph/graph-controller.test.ts src/supervisor/worker-manager.test.ts` reported 53 passing tests.
- Broader execution-graph coverage passed: `npx vitest run src/runtime/execution-graph/graph-controller.test.ts src/runtime/execution-graph/synthesis-node.test.ts src/runtime/execution-graph/mutation-node.test.ts src/runtime/direct-reasoning-mode.test.ts src/supervisor/worker-manager.test.ts` reported 97 passing tests.
- Local gates passed after the cleanup: `npm run check` and `npm run build`.
- Cross-domain and web approval harnesses passed after the cleanup: `node scripts/test-cross-domain-orchestration-stress.mjs` and `node scripts/test-web-approvals.mjs`.

Checkpoint after the worker-suspension continuation approval cleanup:

- `src/runtime/execution-graph/worker-suspension-resume.ts` now owns continuation approval pending-action projection for resumed worker suspensions through `recordWorkerSuspensionGraphContinuationPendingAction`: follow-up interruption event emission, `WorkerSuspension` artifact creation, resume-token projection, and graph pending-action replacement.
- `WorkerManager` still reads the resumed worker metadata, dispatches the continuation worker turn, and supplies pending-action/graph/timeline stores, but it no longer builds the follow-up worker-suspension artifact or graph pending action inline.
- This is an approval/resume ownership cleanup only. It does not change approval policy, worker dispatch, provider/profile selection, filesystem authority, pending-action store ownership, or brokered-worker isolation.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/worker-suspension-resume.test.ts src/supervisor/worker-manager.test.ts` reported 53 passing tests.
- Local gates passed after the cleanup: `npm run check` and `npm run build`.
- Cross-domain and web approval harnesses passed after the cleanup: `node scripts/test-cross-domain-orchestration-stress.mjs` and `node scripts/test-web-approvals.mjs`.

Checkpoint after the delegated-worker approval pending-action cleanup:

- `src/runtime/execution-graph/worker-suspension-resume.ts` now owns initial delegated-worker approval pending-action projection through `recordDelegatedWorkerGraphPendingApprovalAction`: origin-surface scoping, `WorkerSuspension` artifact creation, verification artifact refs, intent projection, and graph pending-action replacement.
- `WorkerManager` still detects delegated approval metadata from worker output, supplies worker/request/target context and stores, and preserves the brokered dispatch boundary. It no longer builds delegated-worker suspension artifacts or graph pending actions inline for the initial approval block.
- This is an approval/resume ownership cleanup only. It does not change approval policy, delegated worker dispatch, provider/profile selection, filesystem authority, pending-action store ownership, or brokered-worker isolation.
- Focused coverage passed: `npx vitest run src/runtime/execution-graph/worker-suspension-resume.test.ts src/supervisor/worker-manager.test.ts` reported 54 passing tests.
- Local gates passed after the cleanup: `npm run check` and `npm run build`.
- Cross-domain and web approval harnesses passed after the cleanup: `node scripts/test-cross-domain-orchestration-stress.mjs` and `node scripts/test-web-approvals.mjs`.

Final scoped completion checkpoint:

- No remaining graph ownership cleanup is currently justified inside this plan without widening scope or extracting cosmetic helper fragments. The remaining meaningful refinement area is synthesis/search answer quality across domains, which should be handled as a separate quality-uplift plan so it does not distort the durable execution graph architecture.
- Final local verification passed after the ownership cleanup series: `npm run check`, `npm run build`, and `npm test` reported 316 files and 3431 passing tests.
...
- Delegated mixed-domain verification now rejects terminal-looking answers that report no repo matches for implementation-location requests when the repo leg has no file refs or implementation claims. This keeps legitimate absent-marker searches valid while forcing retry/targeted repo inspection for weak web+repo+memory synthesis such as "where execution graph mutation approval resume events are emitted".
- Delegated mixed-domain verification also rejects implementation-location answers that cite only repo search-hit files without an implementation-file read. File references backed by actual `fs_read` receipts remain valid, so exact read-only lookups do not become unnecessarily heavy.
- Delegated repo evidence claim construction now treats test/docs/example/fixture reads as support evidence, not implementation-file evidence, unless the task contract explicitly targets those support paths. This keeps mixed-domain implementation-location retries focused on production implementation files without blocking normal test/documentation inspections.
- Delegated job-snapshot evidence refs are now extracted from structured previews such as nested `matches[].relativePath`, `filePath`, `path`, and `files[]` fields. This lets graph verification distinguish search-hit-only repo evidence from read/code-symbol-confirmed implementation evidence even when the evidence arrived through request-scoped delegated job snapshots.
- Delegated approval pending-action projection now rebases serialized worker-suspension source evidence onto the supervisor-verified delegated envelope before persisting the graph artifact. Approval continuation therefore resumes with drained web/repo/memory evidence instead of a stale pre-drain envelope.
- Grounded answer synthesis now tells the no-tools repair pass to cite implementation source files directly for implementation-location answers and not narrate tests, docs, fixtures, examples, or verifier expectations unless the user asked for support artifacts.
- Intent Gateway normalization now derives repo grounding from required planned evidence categories such as `repo_inspect`, even when a structured fallback says `requiresRepoGrounding: false`. This keeps mixed web+repo+memory plans on graph/delegated orchestration while preserving the existing direct preference for ordinary repo read/search turns that do not require tool-backed synthesis.
- Verification for this checkpoint: `npx vitest run src/runtime/intent/structured-recovery.test.ts src/runtime/direct-intent-routing.test.ts src/runtime/execution-graph/delegated-worker-retry.test.ts src/runtime/execution-graph/delegated-worker-verification.test.ts src/runtime/execution/verifier.test.ts src/runtime/worker-suspension.test.ts src/supervisor/worker-manager.test.ts src/worker/worker-session.test.ts --reporter=dot`, `npm run check`, `npm run build`, `node scripts/test-cross-domain-orchestration-stress.mjs`, `node scripts/test-security-verification.mjs`, and `npm test` all passed. The actual app `/api/message` replay with request id `codex-final-mixed-quality-42806` completed through delegated graph retry/verification, returned the three-source answer, cited `src/runtime/execution-graph/mutation-node.ts` for `emitMutationResumeGraphEvent`, found `SMOKE-MEM-42801`, and used managed-cloud Ollama Cloud profiles. It took about 211 seconds after one retry, so latency remains a quality target but correctness and graph verification passed.
