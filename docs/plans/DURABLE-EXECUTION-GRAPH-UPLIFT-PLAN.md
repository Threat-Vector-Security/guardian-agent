...
- Remaining direct-route debt after this checkpoint: coding direct-route dependency builders still live as `ChatAgent` private wrapper methods. The next cleanup should move coding backend/session-control dependency assembly behind an explicit runtime dependency group without changing Intent Gateway routing or brokered-worker authority.
