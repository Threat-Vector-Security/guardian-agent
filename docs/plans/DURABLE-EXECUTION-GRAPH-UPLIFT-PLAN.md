...
- Provider/profile suitability remains uneven across managed-cloud profiles: OpenRouter Kimi handled prior automation-control paths correctly but is currently blocked by account credits in this environment; Ollama Cloud `minimax-m2.1` now proves request-scoped managed-cloud selection and the core chat/approval sweep, while some route-only structured-output weaknesses still need provider/profile verification coverage.
