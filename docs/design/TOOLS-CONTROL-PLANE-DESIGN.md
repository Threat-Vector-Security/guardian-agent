...
- Google Workspace: `gws`, `gws_schema`, `gmail_draft` when native Google integration is enabled
- Microsoft 365: `m365`, `m365_schema`, `outlook_draft`, `outlook_send` when native Microsoft integration is enabled
...
The base always-loaded set is configured through `assistant.tools.deferredLoading.alwaysLoaded` and currently defaults to:
`find_tools`, `web_search`, `fs_read`, `shell_safe`, `memory_search`, `memory_save`

When `assistant.tools.agentPolicyUpdates` is enabled, `update_tool_policy` is appended to the initial tool list.

When native Google or Microsoft integrations are configured, the executor also appends their first-round productivity tools so the model can use them without a discovery round:
`gws`, `gws_schema`, `gmail_draft`
`m365`, `m365_schema`, `outlook_draft`
