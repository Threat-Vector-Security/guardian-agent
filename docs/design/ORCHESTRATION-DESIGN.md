...
- recovery and runtime-proof paths must remain deliverable-neutral: they may name missing evidence, provide retry guidance, and run generic checks, but they must not create target-workspace source files, sample data, UI behavior, domain-specific verification scripts, or canned final answers for the delegated worker
