...
- run and routing views support `continuityKey` and `activeExecutionRef` filters; the System runtime timeline also exposes status, parent-run, execution-lineage, graph-event, agent, channel, and code-session filters
