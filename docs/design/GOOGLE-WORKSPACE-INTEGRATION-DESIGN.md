...
> The older dual-mode config (`assistant.tools.google.mode`) and dedicated CLI wrapper service do not exist in the current codebase.
> Remaining sections below are historical design context only.

---

## Overview

GuardianAgent originally integrated with Google Workspace (Gmail, Calendar, Drive, Docs, Sheets) through the **Google Workspace CLI** (`@googleworkspace/cli`) running as a subprocess. This document captures that earlier CLI-backed design.

The historical architecture:

- a dedicated CLI wrapper service executed the `gws` CLI directly via `child_process.execFile` for each tool call
...
### Legacy CLI Wrapper Service (removed from current codebase)
...
- an auto-provisioning helper script for the CLI install flow
