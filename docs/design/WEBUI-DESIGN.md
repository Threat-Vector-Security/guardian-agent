...
Configuration opens as a modal window in focused mode so setup stays available without displacing the main chat surface. Other navigation entries may use the same modal window pattern when selected from the rail. Focused page windows should be movable, resizable, and maximizable so large tools such as Code can use the full available workspace. Closing the modal returns the operator to the chat-first surface. Classic and workstation shells remain selectable from Configuration > Appearance and must be restorable without reload.
...
The active shell layout should also be switchable from a compact top-right selector so operators can move between Default Chat, Classic, and Web Browser layouts without opening Configuration.
