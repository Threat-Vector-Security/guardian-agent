...
The repo currently ships `scripts/test-cloud-config.mjs` for config/control-plane coverage.

Before this surface should be treated as fully covered, the test harness inventory should also include:

- end-to-end natural-language cloud/hosting scenarios through `POST /api/message`
- direct tool API and approval transport coverage via `/api/tools/run` and `/api/tools/approvals/decision`
- automation scenarios that create and run workflows/tasks using cloud tools
- CLI command-family coverage, approval prompts, history inspection, and error copy
- provider-focused suites for cPanel/WHM, Vercel, and Cloudflare as those adapters expand
