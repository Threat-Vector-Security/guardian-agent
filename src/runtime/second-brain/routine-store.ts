...
export class RoutineStore {
  ...
  listRoutines(): SecondBrainRoutineRecord[] {
    ...
    const rows = this.ctx.db!.prepare(`
      SELECT id, template_id, name, category, enabled_by_default, enabled, trigger_json, config_json, workload_class,
    ...
    return rows.map((row) => ({
      id: String(row.id),
      templateId: row.template_id == null ? undefined : String(row.template_id) as SecondBrainRoutineRecord['templateId'],
      ...
      config: row.config_json == null
        ? undefined
        : JSON.parse(String(row.config_json)) as SecondBrainRoutineRecord['config'],
  ...
  upsertRoutine(routine: SecondBrainRoutineRecord): SecondBrainRoutineRecord {
    ...
    this.ctx.db!.prepare(`
      INSERT INTO sb_routines (
        id, template_id, name, category, enabled_by_default, enabled, trigger_json, config_json, workload_class,
        external_comm_mode, budget_profile_id, delivery_defaults_json, default_routing_bias,
        last_run_at, created_at, updated_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).run(
      ...
      routine.config ? JSON.stringify(routine.config) : null,
