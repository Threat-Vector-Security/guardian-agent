...
import { formatOrchestrationRoleDescriptor } from '../orchestration-role-descriptors.js';
...
export function createAgentDashboardCallbacks(
  ...
  const toDashboardAgentInfo = (inst: AgentInstance): DashboardAgentInfo => {
    ...
    const orchestration = inst.definition.orchestration;
    const orchestrationLabel = formatOrchestrationRoleDescriptor(orchestration);
    return {
      id: inst.agent.id,
      ...
      ...(orchestration?.role ? { orchestrationRole: orchestration.role } : {}),
      ...(orchestrationLabel ? { orchestrationLabel } : {}),
      ...(orchestration?.lenses?.length ? { orchestrationLenses: [...orchestration.lenses] } : {}),
