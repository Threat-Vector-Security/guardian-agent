...
import { findFirstElementInnerHtml, findHtmlElementsByClass, htmlToText } from '../util/html.js';
import type {
  ToolApprovalRequest,
  ToolCategory,
  ToolDecision,
...
const MAX_TOOL_CALLS_PER_CHAIN = 100;
...
export class ToolExecutor {
  ...
  private buildDryRunPreview(toolName: string, args: Record<string, unknown>): string {
    const definition = this.registry.get(toolName)?.definition;
    const customPreview = definition?.dryRunPreview?.(args)?.trim();
    if (customPreview) return customPreview;

    const action = asString(args.action).trim();
    const target = resolveDryRunPreviewTarget(args);
    const toolLabel = definition?.shortDescription ?? definition?.description ?? toolName;
    const clippedToolLabel = sanitizePreview(toolLabel).replace(/[.!?]+$/, '');
    const actionPart = action ? ` action '${action}'` : '';
    const targetPart = target ? ` for ${target}` : '';
    return `Would run ${toolName}${actionPart}${targetPart}: ${clippedToolLabel}`;
  }
...
function resolveDryRunPreviewTarget(args: Record<string, unknown>): string {
  for (const key of ['path', 'url', 'query', 'profile', 'profileId', 'project', 'domain', 'bucket', 'name', 'id']) {
    const value = asString(args[key]).trim();
    if (value) return `${key} '${sanitizePreview(value)}'`;
  }
  for (const key of ['instanceIds', 'files', 'selectedProcessTargetIds', 'selectedCleanupTargetIds']) {
    const values = asStringArray(args[key]);
    if (values.length > 0) return `${values.length} ${key}`;
  }
  return '';
}
