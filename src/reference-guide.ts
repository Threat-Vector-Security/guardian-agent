...
export const SECURITY_WORKSPACE_GUIDE = {
  ...
  workspace: 'Use Collapse sidebar or Expand sidebar on desktop; the browser remembers your choice. The header shows the loaded app version. Reload page fetches the current app build and warns about unsaved drafts. Creating, importing or selecting a system updates its address so reload and browser navigation reopen that system. Portable file exports use the native save picker where supported. Other browsers offer a browser-download choice, whose destination follows browser settings.',
            ...
            sections: [
              ...
              },
              {
                ...
                items: [
                  ...
                  'Use the top-right UI layout selector, or Configuration > Appearance > Shell Layer, to choose Default Chat, Classic Layer, or Workstation Layer. Default Chat keeps chat full-height with a compact side rail and opens page windows that can be moved, resized, maximized within the usable workspace, restored locally, and reset by double-clicking the page-window titlebar.',
            ...
            sections: [
              {
                ...
                items: [
                  ...
                  'Configuration > Second Brain owns the editable setup preferences for assistant response style, local timezone, the Today setup card, and default delivery channels for new routines. It is not a separate memory system.',
              ...
              },
              {
                ...
                items: [
                  ...
                  'The bundled default timezone is `Australia/Brisbane`; change Configuration > Second Brain > Local Timezone if your local day is elsewhere.',
                  'Set Configuration > Second Brain > Local Timezone, for example `Australia/Brisbane`, when Guardian runs on a hosted server so Morning Brief and other scheduled routines follow your local day instead of the server clock.',
              ...
              },
              {
                ...
                items: [
                  ...
                  'Chat-driven local calendar, task due-date, contact last-contact updates, brief date labels, and routine schedules use the configured Second Brain timezone when set.',
            ...
            sections: [
              {
                ...
                items: [
                  ...
                  'Use the web New Conversation control, CLI `/reset [agentId]`, or Telegram `/reset [agentId]` to clear context. In CLI, `/reset` clears the current conversation; pass an agent id such as `/reset default` only when you want to target a specific agent.',
              ...
              },
              {
                ...
                items: [
                  ...
                  'Ordinary coding-session delegated work returns inline to the active chat. Explicit background coding runs can still appear as long-running in runtime job filters, while delegated work started by automations appears as automation-owned.',
                  'Web chat uses safe operational activity labels such as route/context checks, model work, and provider/tool waits before detailed timeline events arrive. These labels are status signals, not hidden chain-of-thought.',
                ...
                note: 'Web chat and CLI show live request progress while the assistant is working. The running feed now rolls forward instead of stacking every step, while blocked or otherwise important milestones remain visible with the final reply when you still need them. Telegram keeps these updates concise because it is better suited to remote follow-up than dense live logs.',
              },
            ],
          },
          {
            ...
            sections: [
              {
                ...
                items: [
                  ...
                  'Held results can be replayed, deferred, or dismissed directly from `Agent Runtime`; when context is available, the replay keeps the related execution, continuity, and coding-session references attached.',
                  'Deferred held results can show a review-after time when one is supplied, which makes later operator review easier to scan without changing the held result content.',
                  'Held-result replay, defer, and dismiss actions can also appear in `Runtime Execution` as short timeline notes on the related run.',
                  ...
                  'Graph-backed Runtime Execution rows can show bounded facts such as result status, verification decision, artifact trust, run class, reporting mode, worker id, and task-run id when those details are available.',
              ...
              },
              {
                ...
                items: [
                  ...
                  'Use `/assistant jobs followup <jobId> <replay|defer|dismiss> [deferMinutes]` when a held result is waiting for operator review. The optional minutes value applies only to `defer`.',
            ...
            sections: [
              {
                ...
                items: [
                  'Create a code session either from Guardian chat or from `#/code` by choosing a title plus the project folder you want Guardian to anchor to.',
                  'In the web Code page on cloud or container deployments, leaving the project folder blank creates a title-based folder under the durable projects directory. Browse when you want to select an existing folder instead.',
            ...
            sections: [
              ...
              },
              {
                ...
                items: [
                  ...
                  'For a remote or always-on Guardian host, configure the Google OAuth client with your Guardian callback URL, for example `https://guardian.example.com/api/google/auth/callback`, set that same URL as `assistant.tools.google.oauthRedirectUri`, then restart Guardian before connecting.',
            ...
            sections: [
              ...
              },
              {
                ...
                items: [
                  ...
                  'For a remote or always-on Guardian host, add a Web redirect URI such as `https://guardian.example.com/api/microsoft/auth/callback`, set that same URL as `assistant.tools.microsoft.oauthRedirectUri`, then restart Guardian before connecting.',
            ...
            sections: [
              {
                ...
                items: [
                  ...
                  'Held delegated-result actions such as replay, defer, and dismiss are available from System or CLI `/assistant jobs followup`, and Guardian audits the acting surface when it is available. A deferred result may also show a review-after time when the operator supplies one.',
                  'System > Runtime Execution can filter delegated timelines by run class, reporting mode, and blocker kind when you need to isolate long-running, held, or approval-blocked work.',
