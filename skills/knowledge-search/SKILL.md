...
2. Read only the specific files or passages you need after the search narrows the scope.
   - Use `fs_read` for exact follow-up reads, including direct text extraction from a known local PDF path.
...
- For one specific local PDF file, prefer `fs_read`; for many PDFs or broad discovery, index them and use `doc_search`.
